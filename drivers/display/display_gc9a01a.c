...
int gc9a01a_regs_init(const struct device *dev) {
    ...
    while ((cmd = *addr++) > 0) {
        ...
        addr += numArgs;
        if (x & 0x80) {
            k_msleep(GC9A01A_SLEEP_OUT_TIME + 30); // 30ms on top of the 120ms sleepout time to
...
#define GC9A01A_INIT(n, t)                                                                         \
    ...
    static const struct gc9a01a_config gc9a01a_config_##n = {                                      \
        .spi = SPI_DT_SPEC_GET(INST_DT_GC9A01A(n), SPI_OP_MODE_MASTER | SPI_WORD_SET(8), 0),       \
        .cmd_data = GPIO_DT_SPEC_GET(INST_DT_GC9A01A(n), cmd_data_gpios),                          \
        .reset = GPIO_DT_SPEC_GET_OR(INST_DT_GC9A01A(n), reset_gpios, {0}),                        \
        .backlight = GPIO_DT_SPEC_GET_OR(INST_DT_GC9A01A(n), backlight_gpios, {0}),                \
    ...
    DEVICE_DT_DEFINE(INST_DT_GC9A01A(n), gc9a01a_init, NULL, &gc9a01a_data_##n,                    \
                     &gc9a01a_config_##n, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY, &gc9a01a_api)
